...
#
# Parameters:
#   namevar - ignored, there can only be one network plugin specification per
#         node
  ...
  collectd::plugin{
    'network':
      lines => [
        $listen ? {
          ''      => '',
          default => regsubst($listen, '^(.*)$', 'Listen \1', 'G'),
        },
        $server ? {
          ''      => '',
          default => regsubst($server, '^(.*)$', 'Server \1', 'G'),
        },
        $ttl ? {
          ''      => '',
          default => "TimeToLive ${ttl}",
        },
        $forward ? {
          ''      => '',
